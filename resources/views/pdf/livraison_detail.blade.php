...
<hr style="margin-top:20px;"/>
        ...
        <tr>
            <td style="width:70%">
                <p style="font-size: 12px">
                    <strong>DETAIL DE LA LIVRAISON   </strong> <br> <br>
                    {{-- <strong>N de bon :   </strong> {{$bonItem?->code}}<br> --}}

                    <strong>Reference : </strong>
                    {{$items?->reference }}  <br>
                    <strong>Statut de la livraison : </strong>
                    @if ($items?->status == 1)
            ...
            <td style="width:30%">
                <p style="font-size: 12px">
                    @if ($items->delivery_person)
                    <strong >INFORMATION DU LIVREUR</strong> <br> <br>
                    <strong> Nom et Prénoms : </strong>
                    {{ucfirst($items?->delivery_person?->first_name) }} {{ucfirst($items?->delivery_person?->last_name) }}
                    <br>
                    <strong>Email : </strong>{{$items?->delivery_person?->email}}
                    <br>
                    <strong>Contact : </strong>{{$items?->delivery_person?->phone}}
                    <br>
    ...
    @if ($items?->delivery_details->count())
    <table id="invoice" style=" border : solid 1px #c6c4c6; font-size : 12px; margin-top: 0px !important;" class="tableau ">
        ...
        <tbody>
            @foreach($items->delivery_details as $item)
